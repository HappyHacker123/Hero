...
FILE = 'data_issues_report(Hero+).xlsx'
...
def diagnosis(request):
    sys.path.insert(0, "/root/Hero/Myprojects/Hero-new")
    from test import outer_detect
    repo_name = request.GET.get("repo_name")
    repo_version = request.GET.get("repo_version")
    if repo_name and repo_version:
        # try:
        report_l, dep_tree, project_info = outer_detect(repo_name, repo_version, False)
        msg = "testing 4"
        root_cause = ""
        solution = ""
        for i in report_l:
            ...
            solution += i[1] + "</br>"
        if not dep_tree:
            data_dir = {'d_r': repo_name, 'd_m': 3, 'u_r': repo_name,
                        'u_m': 3}
            dep_tree = [json.dumps(data_dir)]
        return JsonResponse({'result': 0, 'data': msg, 'dep_tree': json.dumps(dep_tree),
                             'root_cause': root_cause, 'solution': solution, 'project_info': project_info})
        # except Exception as exp:
        #     print("*****")
        #     print(exp)
        #     print("*****")
        #     msg = 'Please check the input!'
        #     return JsonResponse({'result': -1, 'data': msg})
    else:
        insert_error = -3
        msg = 'Please wait!'
        return JsonResponse({'result': insert_error, 'data': msg})